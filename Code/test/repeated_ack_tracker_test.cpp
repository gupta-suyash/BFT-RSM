                       ...
                       std::vector<std::unique_ptr<AcknowledgmentTracker>>* const ackTrackers)
{
    ...
    BOOST_TEST_MESSAGE("Outside: " << initialMessageTrack << " " << finialMessageTrack << " " << nodeId << " " << nodeAckView.ackOffset << " " << curQuack.value_or(545454545));
    for (uint64_t curMessage = initialMessageTrack; curMessage <= finialMessageTrack; curMessage++)
    {
        BOOST_TEST_MESSAGE("Inside: " << curMessage << " " << nodeId << " " << nodeAckView.ackOffset << " " << curQuack.value_or(545454545));
