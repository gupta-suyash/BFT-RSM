...
def executeRemoteCommandNoCheck(host, command, key=None):
    if not key:
        cmd = "ssh -o StrictHostKeyChecking=no -t " + host + " \"" + command + "\""
    else:
        cmd = "ssh -o StrictHostKeyChecking=no -t -i " + key + " " + host + " \"" + command + "\""
    print("[" + cmd + "]")
    executeCommand(cmd)
...
# on each of the hosts in the list
# If returns an error throws an exception
def executeParallelBlockingDifferentRemoteCommands(hosts, commands, key=None):
    thread_list = list()
    for i in range(0, len(hosts)):
        ...
        if hosts[i] == IPAddr:
            ...
            cmd = "ssh -o StrictHostKeyChecking=no -t " + hosts[i] + " '" + commands[i] + "'"
        else:
            cmd = "ssh -o StrictHostKeyChecking=no -t -i " + \
