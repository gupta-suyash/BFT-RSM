...
experiment_name="omg_its_all_ok"
	...
	for ack_window in "${ack_windows[@]}"; do
		...
		if [ $kafka = "true" ]; then
			...
			for node in $(seq 0 $((rsm1_size - 1))); do
				print_kafka_json "config.json" "topic-1" "topic-2" "1" "${node}" "5" "${read_from_pipe}" "${file_100}" "60" "120" "0" "/tmp/scrooge-input" "/tmp/scrooge-output" "${broker_ips_string}" "${run_dr}" "${run_ccf}"
			...
			for node in $(seq 0 $((rsm2_size - 1))); do
				print_kafka_json "config.json" "topic-1" "topic-2" "2" "${node}" "5" "${read_from_pipe}" "${file_100}" "60" "120" "0" "/tmp/scrooge-input" "/tmp/scrooge-output" "${broker_ips_string}" "${run_dr}" "${run_ccf}"
