...
stake_splits=(1 2 4 8 16 32 64 128)
for stake_split in "${stake_splits[@]}"; do
	RSM1_Stake[0]="${stake_split}"
	RSM2_Stake[0]="${stake_split}"
...
for r1_size in "${rsm1_size[@]}"; do # Looping over all the network sizes
	rsm1_fail[0]="$(((stake_split + r1_size - 1) / 3))"
	rsm2_fail[0]="$(((stake_split + r1_size - 1) / 3))"
