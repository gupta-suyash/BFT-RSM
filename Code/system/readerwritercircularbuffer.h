...
template <typename T> class BlockingReaderWriterCircularBuffer
{
  ...
  public:
    explicit BlockingReaderWriterCircularBuffer(std::size_t capacity)
        : maxcap(capacity), mask(), rawData(), data(),
          ...
          items(new spsc_sema::LightweightSemaphore(0)), nextSlot(0), nextItem(0)
    {
    ...
    BlockingReaderWriterCircularBuffer(BlockingReaderWriterCircularBuffer &&other)
        : maxcap(0), mask(0), rawData(nullptr), data(nullptr), slots_(new spsc_sema::LightweightSemaphore(0)),
          items(new spsc_sema::LightweightSemaphore(0)), nextSlot(), nextItem()
    {
        swap(other);
    }
    ...
    {
        return wait_enqueue_timed(std::move(item),
  ...
  private:
    template <typename U> void inner_enqueue(U &&item)
    {
    ...
    template <typename U> void inner_dequeue(U &item)
    {
  ...
  private:
    ...
    std::unique_ptr<spsc_sema::LightweightSemaphore>
        slots_; // number of slots currently free (named with underscore to accommodate Qt's 'slots' macro)
