...
{
template <uint64_t kViewSize> struct AckView
{
...
template <uint64_t kViewSize> bool testAckView(const AckView<kViewSize> &ackView, const uint64_t ack)
{
...
template <uint64_t kViewSize> uint64_t getFinalAck(const AckView<kViewSize> &ackView)
{
    if constexpr (kViewSize != 0)
    {
        for (int64_t i = ackView.view.size() - 1; i >= 0; i--)
        {
            ...
            if (numRighZeros != 64)
            {
                return std::min<uint64_t>(ackView.ackOffset + kTrueKlistSize - 1,
                                          ackView.ackOffset + (i * 64) + (64 - numRighZeros) - 1);
            }
        }
    }
...
template <uint64_t kViewSize> std::optional<uint64_t> getAckIterator(const AckView<kViewSize> &ackView)
{
