                                                            ...
                                                            const uint64_t ackValue)
{
    ...
    mQuorumAck = curQuorumAck;
    return curQuorumAck;
}

void RelaxedQuorumAcknowledgment::reset()
{
    mNodeToAck.clear();
    mAckToStakeCount.clear();
    mQuorumAck = std::nullopt;
    mStakeInCurQuorum = 0;
}

uint64_t RelaxedQuorumAcknowledgment::getStakeAtAck(uint64_t ack) const
{
...
std::optional<uint64_t> RelaxedQuorumAcknowledgment::getNodeAck(const uint64_t nodeId) const
{
 ...
 * @return mQuorumAck
 */
std::optional<uint64_t> RelaxedQuorumAcknowledgment::getCurrentQuack() const
{
    return mQuorumAck;
}
